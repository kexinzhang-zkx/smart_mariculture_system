...
static lv_obj_t *create_card(lv_obj_t *parent, const char *title,
    const char *img_path, lv_obj_t **val_label)
{
...
void ui_Screen4_screen_init(void)
{
    ...
    create_card(row1, "温度", "A:/温度 (1).png", &ui_SensorTemp);
    create_card(row1, "湿度", "A:/湿度.png", &ui_SensorHumi);
    create_card(row1, "光照", "A:/光照.png", &ui_SensorLight);

    lv_obj_t *row2 = create_row(ui_Container35);
    create_card(row2, "溶解氧", "A:/溶解氧.png", &ui_SensorDO);
    create_card(row2, "pH 值", "A:/PH值.png", &ui_SensorPH);
    create_card(row2, "浊度", "A:/浊度.png", &ui_SensorTurb);
